...
def get_weaviate_client():
    """Tạo kết nối tới Weaviate Cloud (WCS) dựa trên biến môi trường."""
    import weaviate
    ...
    if not weaviate_url or not weaviate_api_key:
        raise ValueError("WEAVIATE_URL và WEAVIATE_API_KEY phải được thiết lập cho Weaviate Cloud")
...
def embed_chunks(chunks: list[dict]) -> list[dict]:
    ...
    if not openai_key:
        raise ValueError("OPENROUTER_API_KEY (hoặc OPENAI_API_KEY alias) không được thiết lập trong môi trường / file .env")
    ...
    for i in range(0, len(texts), batch_size):
        ...
        embeddings = [row["embedding"] for row in payload.get("data", [])]
        if len(embeddings) != len(batch_texts):
            raise RuntimeError("OpenRouter trả về số embedding không khớp với số chunks")
...
def index_to_vectorstore(chunks: list[dict]):
    ...
    try:
        rebuild = os.getenv("REBUILD_WEAVIATE_INDEX", "").lower() in {"1", "true", "yes"}
        if client.collections.exists("DrugLawDocs"):
            if rebuild:
                ...
                print("Đang dùng collection 'DrugLawDocs' hiện có trên Weaviate.")
        else:
            collection = client.collections.create(
                name="DrugLawDocs",
